...
pub(crate) fn whois_toy_impl() -> Result<(), ParseError> {
    ...
    // let whois = WhoIs::from_path("data/whois.json").expect("Failed to load whois.json");
    // let server_name =
    //     data::whois_server_for(domain_name.to_string()).expect("Failed to get server name");
    // let whois = WhoIs::from_host(server_name.as_str()).expect("Failed to contact WHOIS server");
    let whois =
