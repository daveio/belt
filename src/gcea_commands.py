...
def read_config(filename):
    ...
    try:
        with open(filename, "r") as conf_file:
            ...
            # token: str
            procd = dict(
                [[token.strip() for token in line.split("=")] for line in lines]
            )
            return procd
    except FileNotFoundError:
        print(
            "{0} not found. copy gcea.conf.example to {0}, edit, and retry".format(
                filename
            )
        )
        exit(1)
...
def cli():
    ...
    print(
        "               {0} | {1} videos total, {2} videos to consider".format(
            msg, len(vids), len(vids_pending)
        )
    )
    time.sleep(1)
    for vid in vids_pending:
        try:
            ...
            if err.type == "MAX_PENDING_CONVERSIONS":
                ...
                print(
                    "           {0} | {1}{2}{3}".format(
                        msg, style.DIM, vid.name, style.RESET_ALL
                    )
                )
                exit(1)
            elif err.type == "ConversionNotNeeded":
                msg = "{0}Conversion not needed{1}".format(
                    fore.MAGENTA, style.RESET_ALL
                )
                print("{0} | {1}".format(msg, vid.name))
            elif err.type == "NotVideo":
                msg = "{0}File is not a video{1}".format(fore.MAGENTA, style.RESET_ALL)
                print("  {0} | {1}".format(msg, vid.name))
            else:
                pprint.pprint(err)
        finally:
            if not exception_occurred:
                msg = "{0}Conversion started{1}".format(fore.GREEN, style.RESET_ALL)
                print("   {0} | {1}".format(msg, vid.name))
    exit(0)
